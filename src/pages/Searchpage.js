...
function Searchpage() {
  ...
  return (
    <div className="covidData">
     ...
     {entry ?<> <p>Country Name : {about.Country} </p>
         <p>TotalCase : {about.TotalCases}</p>
  
        <p>Active Cases : {about.ActiveCases}</p>
  
        <p>Recovered Cases : {about.TotalRecovered}</p>
        <p>Total Deaths : {about.TotalDeaths}</p> </>:<> Please Enter Country Name</>}
    </div>
    
    {entry?<><div className="chart" style={{ width: "300px",marginTop: "10%",marginLeft: "15%"}} ><Pie 
    
    
    data={{ labels:["TotalCases","ActiveCases","TotalDeaths","RecoveredCases"],
